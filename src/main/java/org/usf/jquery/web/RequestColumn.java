...
public final class RequestColumn {
	...
	public DBColumn dbColumn(){
		return tableDecorator().column(cd);
	}
	
	public DBOrder dbOrder(){
		return tableDecorator().column(cd).order(exp); //check value
	}
	...
	static RequestColumn decodeColumn(String value, TableDecorator defaultTable, boolean allowedExp) {
		...
		if(arr.length > 1 && allowedExp && !context().isDeclaredColumn(arr[arr.length-1]) && lookupFunction(arr[arr.length-1]).isEmpty()) {//!function && !column
	...
	static Stream<RequestColumn> decodeColumns(String value, TableDecorator defaultTable, boolean allowedExp) {
		return LinkedRequestEntry.parseEntries(value)
				.stream()
	...
	private static RequestColumn decode(String[] arr, int index, TableDecorator defaultTable, String exp, String tag) {
		...
		var fn = lookupFunction(value).orElseThrow(()-> cannotEvaluateException("column expression", value)); //function expected
	...
	public static void main(String[] args) {
		parseEntries("count()");
		parseEntries("col1");
		parseEntries("t1.co_l1");
		parseEntries("col1.avg");
		parseEntries("t1.col1.avg");
		parseEntries("count().string(usf.ff).substring(2,6.6).gt");
		parseEntries("col.func().string(10).substring(2,6.6).order");
		parseEntries("view.row_number.over(partition:co1,col2,order:col2):mm");
		parseEntries("t1.col1");
		parseEntries("ab,xy");
		parseEntries("t1:a,xy");
		parseEntries("t1(aa,kkk,123):aac,xy:R");
	}

...
public final class BarChartView implements ResultWebView {
    ...
    private StringBuilder[] simpleMap(ResultSet rs) throws SQLException { //2d
    	...
    	sb1.append("[")
    	.append(quote(xType.typeName())).append(",")
    	.append(quote(cols > 1 ? rsm.getColumnName(1) : ""))
    	.append("],[")
    	.append(quote(yType.typeName())).append(",")
    	.append(quote(rsm.getColumnName(cols)))
    	.append("]");
	...
	private StringBuilder[] pivotAndMap(ResultSet rs) throws SQLException { //xAxis,dim1,...,yAxis 
    	...
    	sb1.append("[")
    	.append(quote(xType.typeName())).append(",")
    	.append(quote(rsm.getColumnName(1)))
    	.append("]");
    	headers.forEach(s-> sb1.append(",[").append(quote(NUMBER.typeName())).append(",").append(quote(s)).append("]"));
    	var sb2= new StringBuilder();

...
public final class StoreManager {
	...
	private final Map<Class<?>, StoreCatalog> stores = new LinkedHashMap<>();
	
	public static StoreManager getInstance() {
		return instance;
	}
	
	public void register(Class<? extends StoreCatalog> clazz, DataSource ds) {
		stores.put(clazz, createStore(clazz, ds));
	}
	
	public void register(Class<? extends StoreCatalog> clazz, DataSource ds, Dialect dialect) {
		stores.put(clazz, createStore(clazz, ds, dialect));
	}
	
	public <T extends StoreCatalog> T getStore(Class<T> clazz){
	...
	public <T extends StoreCatalog> T getDefaultStore(){
		if(stores.size() == 1) {

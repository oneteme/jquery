...
public abstract interface DataProvider {
		...
		return IntStream.range(1, 5)
			.mapToObj(n->{
				...
				for(int i=0; i<n; i++) {
					...
					coList.add(col);
					exList.add(new ColumnSingleFilter(col, new OperatorSingleExpression<>((CompareOperator)arg.get()[0], arg.get()[1])));
	...
	static Stream<Arguments> filterCaseProvider() {
		...
		return operationCaseProvider()
			.map(a->
				of(new ColumnSingleFilter(COLUMNS[rand.nextInt(4)], new OperatorSingleExpression<>((CompareOperator)a.get()[0], a.get()[1])), a.get()[2]));
	}
